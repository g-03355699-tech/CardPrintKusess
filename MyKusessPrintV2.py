...
class CardPrinterApp(ctk.CTk):
    def __init__(self):
        ...
        self.title("Sistem Cetakan Kad MyKUSESS (Versi Save Layout + Tick Selection)")
        ...
        selection_btn_frame = ctk.CTkFrame(self.tab_batch, fg_color="transparent")
        selection_btn_frame.pack(fill="x", pady=2)
        
        ctk.CTkButton(selection_btn_frame, text="☑ Pilih Semua", fg_color="#2ecc71", hover_color="#27ae60", width=140, command=self.select_all_records).pack(side="left", padx=(0, 10))
        ctk.CTkButton(selection_btn_frame, text="☐ Nyahpilih Semua", fg_color="#e74c3c", hover_color="#c0392b", width=140, command=self.deselect_all_records).pack(side="left")
        ...
        # Menambahkan kolum "Pilih" pada Treeview
        self.tree = ttk.Treeview(self.tab_batch, columns=("Pilih", "QR", "Panggilan", "Penuh"), show="headings", height=8)
        ...
        self.tree.column("Pilih", width=50, anchor="center")
        self.tree.column("QR", width=90)
        self.tree.column("Panggilan", width=120)
        self.tree.column("Penuh", width=250)
        self.tree.pack(fill="both", expand=True, pady=5)
        
        # Menukar binding event kepada click release untuk pengurusan tickbox
    ...
    def load_csv(self):
        ...
        try:
            with open(csv_path, mode='r', encoding='utf-8') as f:
                ...
                for row in reader:
                    # Menambahkan status default ditandakan (True) bagi setiap rekod baru
                    row['pilih'] = True
    ...
    def refresh_treeview(self):
        # Memadam data lama dalam visual senarai Treeview
        self.tree.delete(*self.tree.get_children())
        for row in self.csv_data:
            pilih_symbol = "☑" if row.get('pilih', True) else "☐"
            self.tree.insert("", tk.END, values=(pilih_symbol, row['kod_qr'], row['nama_panggilan'], row['nama_penuh']))
    ...
    def on_tree_click(self, event):
        """Menguruskan aksi klik pada senarai murid."""
        ...
        # Cari rekod asal dalam data CSV
        record = next((r for r in self.csv_data if r['kod_qr'] == qr_val), None)
        if not record: return
        
        if column == "#1":
            # Jika user klik pada ruangan kolum Tickbox, ubah status pilihannya
            record['pilih'] = not record.get('pilih', True)
            self.refresh_treeview()
            # Kekalkan focus pada item yang diklik
            self.tree.focus(selected_item)
            self.tree.selection_set(selected_item)
        else:
            # Jika klik pada bahagian nama, jana paparan preview
            try:
    ...
    def trigger_refresh_preview(self):
        ...
        if selected_items:
            ...
            # values[1] adalah kolum QR (kerana kolum index 0 adalah 'Pilih')
    ...
    def print_batch(self):
        # Hanya ambil murid yang ditandakan ☑ (pilih=True)
        selected_records = [r for r in self.csv_data if r.get('pilih', False)]
        if not selected_records: 
            messagebox.showwarning("Peringatan", "Tiada murid yang ditandakan ☑ untuk dicetak!")
            return
